// git.status = dirty, build.date = Mon Jul 29 11:54:04 EDT 2024, git.hash =
// 9ec9a58
#include "chrono"
#include "parser.cpp"
/***************** Parse helpers  ******************/
...
#pragma HLS loop_tripcount min = 0 max = 7 avg = 4
    ...
    times[z] = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
                   .count();
  }
  long sum = 0;
  for (auto ii = 0; ii < numRuns; ii++) {
    std::cout << times[ii];
    if (ii == numRuns - 1) {
      std::cout << "\n";
    } else {
