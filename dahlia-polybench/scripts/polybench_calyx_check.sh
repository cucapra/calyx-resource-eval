...
ls benchmarks/polybench/*.fuse | parallel --bar $parallelism "fud e -q {} --to dat --through verilog -s verilog.data '{}.data' -s calyx.flags '-i static-promotion:cell-share' | diff -q <(jq .memories) benchmarks/polybench-expect/{/.}.expect >> results-diff.txt"
