...
def move_file(file_directory, base_name, suffix, extension, dst):
    """copies file at [file_directory]/[base_name][suffix].[extension]
    and moves it to dst
    Args:
    """
...
if __name__ == "__main__":
    ...
    for benchmark in run_settings["benchmarks"]:
        ...
        for suff in run_settings["mlir_files_suffix"]:
            move_file(mlir_dir, f"{benchmark}-main", suff, "mlir", run_settings["dst"])
        for suff in run_settings["futil_files_suffix"]:
            move_file(futil_dir, f"{benchmark}", suff, "futil", run_settings["dst"])
        for file_name in run_settings["data_files"]:
            move_file(data_dir, "", file_name, "json", run_settings["dst"])
